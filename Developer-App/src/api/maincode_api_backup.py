...
class MainCodeAPI:
    ...
    def get_all_main_codes(self, record_type: Optional[str] = None,
        ...
        try:
            with self.db_manager.get_cursor() as cursor:
                ...
                # Get total count                count_query = "SELECT COUNT(*) FROM Maincode"
                count_params = []
                if record_type:
    ...
    def update_main_code(self, original_code: str, update_data: Dict[str, Any],
        ...
        try:
            ...
            # Build update query
            update_clauses = []
            values = []            for field, value in update_data.items():
            ...
            Logger.info(f"Main code updated: {original_code} -> {new_code}")
    ...
    def delete_main_code(self, code: str, record_type: Optional[str] = None) -> ApiResponse:
        ...
        try:
            # Check if main code exists
            existing_response = self.get_main_code(code, record_type)
            if not existing_response.success:
                return existing_response

            with self.db_manager.get_cursor() as cursor:
                if record_type:                    cursor.execute(
    ...
    def search_main_codes(self, search_term: str, record_type: Optional[str] = None) -> ApiResponse:
        """Search main codes by term in code, name, or description"""
        try:
            # Build search query
            search_clauses = ["code LIKE ?", "name LIKE ?", "description LIKE ?"]
    ...
    def bulk_insert_main_codes(self, codes_data: List[Dict[str, Any]]) -> ApiResponse:
        """Insert multiple main codes at once"""
        try:
            if not codes_data:
                return ApiResponse.error_response("No data provided for bulk insert")
            ...
            for code_data in codes_data:
                ...
                insert_data.append(list(main_code.to_dict().values()))
