...
class MockDatabaseAPI:
    ...
    def get_main_codes_by_type(self, code_type):
        ...
        return []    def get_cities_by_province(self, province_code):
        ...
        # But actually, looking at the data: 001 -> 00101, so the pattern is just province_code + something
        # Let's check: Iraq '001' -> Baghdad '00101' - the pattern should be '001' at the start of '00101'
        prefix_pattern = f"00{province_code[1:]}"  # Remove the first '0' from '001' to get '01', then '0001'

        print(f"DEBUG: All available cities:")
        for city in all_cities:
            print(f"DEBUG: - {city.get('code')} - {city.get('name')}")

        print(f"DEBUG: Province code: '{province_code}'")
        ...
        for city in all_cities:
            ...
            # Actually let's try a different approach - check if city code contains the province pattern
            # Iraq '001' -> cities should be '00101', '00102' etc.
            # So '001' should match with '00101' - the '001' appears at position 2-4 in '00101'
            if len(city_code) >= 5 and city_code[2:5] == province_code:
